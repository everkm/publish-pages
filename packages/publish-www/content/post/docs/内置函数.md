...
{{daobox::include(file="post_meta_schema.csv", as="table")}}
...
{{daobox::include(file="post_detail_schema.csv", as="table")}}
...
--- | --- | --- | --- | --- | 
url | 本地地址 | 字符串 |  | 
width | 图片宽度 | 整数 | :heavy_check_mark: |  仅资源为图像时可用
height | 图片高度 | 整数 | :heavy_check_mark: |  同上
...
{{daobox::include(file="media_dimension_schema.csv", as="table")}}
